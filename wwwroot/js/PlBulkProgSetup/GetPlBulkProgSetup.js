...
$(function () {
    $("#table").dataTable({
        ...
        ajax: {
            "async": "true",
            "url": "/PlBulkProgSetup/GetTableData?path=" + window.location.pathname,
            "type": "POST",
            "datatype": "json"
        },
        columnDefs: [
        ...
        columns: [
                ...
                "name": "PROGNO", "autoWidth": true },
            {
                "render": function (data, type, row) {
                    var detailsUrl =
                        `<a href='/PlBulkProgSetup/DetailsPlBulkProgSetup?id=${row.encryptedId
                            }' class='fa fa-info-circle fa-2x-custom text-info mr-2'></a>`;
                    var editUrl =
                        `<a href='/PlBulkProgSetup/EditPlBulkProgSetup?id=${row.encryptedId
                        }' class='fa fa-edit fa-2x-custom text-warning mr-2'></a>`;
                    var deleteUrl =
                        `<a href='/PlBulkProgSetup/DetailsPlBulkProgSetup?id=${row.encryptedId
                        }' class='fa fa-trash fa-2x-custom text-danger mr-2'></a>`;

                    //if (data.iS_CLOSED_THE_SO == true) {
                    //    return `${detailsUrl} ${deleteUrl}`;
                    //} else {
                    //    return `${detailsUrl} ${editUrl} ${deleteUrl}`;
                    //}
                    return (row.iS_CLOSED === false) ? `${detailsUrl} ${editUrl} ${deleteUrl}` : `${detailsUrl} ${deleteUrl}`;
