...
$(function () {
    ...
    $("#PlBulkProgYarnD_LOTID").on("change", function () {
        const selectedVal = $(this).val();
        if (selectedVal === "") {
            $("#supplier").text("");
            //$("#yarnFor").text("");
...
function AddProgramSetNoDetails() {
    ...
    $.post("/PlBulkProgSetup/AddProgramSetNoDetails", formData, function (partialView, status, xhr) {
        setQty(xhr);
...
function getSoDetails(orderNo) {
    ...
    const url = "/PlBulkProgSetup/GetRndPoDetails";
