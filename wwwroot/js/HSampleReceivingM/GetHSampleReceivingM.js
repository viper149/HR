$(function () {
    ...
    $("#table").dataTable({
        ...
        ajax: {
            "async": "true",
            "url": "/HSampleReceivingM/GetTableData",
            "type": "POST",
            "datatype": "json"
        },
        columnDefs: [{
        ...
        columns: [
            ...
            {
                "render": function (data, type, row) {
                    return `<a href='/HSampleReceivingM/DetailsHSampleReceivingM?hsrId=${row.encryptedId}' class='fa fa-info-circle fa-2x-custom text-info mr-2'></a><a href='/HSampleReceivingM/EditHSampleReceivingM?hsrId=${row.encryptedId}' class='fa fa-edit fa-2x-custom text-warning mr-2'></a><a href='#' class='fa fa-trash fa-2x-custom text-danger mr-2' onclick=deleteSwal(event,'${row.encryptedId}');></a>`;
                }
            }]
    });
});
...
function deleteSwal(e, item) {
    ...
    swal({
        ...
        function (isConfirm) {
            if (isConfirm) {
                window.location = `/HSampleReceivingM/DeleteHSampleReceivingM?hsrId=${item}`;
            }
        });
}