...
{
    public class AccPhysicalInventoryFabController : Controller
    {
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly IACC_PHYSICAL_INVENTORY_FAB _accPhysicalInventoryFab;
        private readonly IDataProtector _protector;
        private readonly string title = "Physical Inventory (Fabric)";

        public AccPhysicalInventoryFabController(IDataProtectionProvider dataProtectionProvider,
        ...
        [HttpGet]
        public IActionResult CreateAccPhysicalInventoryFab()
        {
            try
            {
                return View();
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAccPhysicalInventoryFab(AccPhysicalInventoryFabViewModel accPhysicalInventoryFabViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (await _accPhysicalInventoryFab.InsertByAsync(accPhysicalInventoryFabViewModel.AccPhysicalInventoryFab))
                {
                    ...
                    return RedirectToAction(nameof(CreateAccPhysicalInventoryFab), "AccPhysicalInventoryFab");
                    
                }

                TempData["message"] = $"Failed to Add {title}";
                TempData["type"] = "error";
                return View(nameof(CreateAccPhysicalInventoryFab));
            }
        ...
        {
            var redirectToActionResult = RedirectToAction(nameof(CreateAccPhysicalInventoryFab), $"AccPhysicalInventoryFab");
